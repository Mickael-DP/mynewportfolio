...
- **Design system**: "Bold & Coloré" — white background, near-black ink, cobalt blue primary accent (`--color-accent`) with a coral secondary pop (`--color-highlight`) used sparingly, `Archivo` (black-weight display) + `Inter` (body) + `JetBrains Mono` (labels/tags), sharp corners by default with `rounded-sm`/`rounded-full` opted in on buttons, pills and cards. Tokens live in `src/styles/global.css` under `@theme` (Tailwind v4's CSS-first config — there is no `tailwind.config.js`). Projects render as a bento grid (`ProjectGrid.astro` defines a 6-slot span/tone/text pattern cycled over the collection) rather than a stacked list.
